...
interface ProjectPageProps {
  searchParams: Promise<{ cwd?: string; sessionId?: string; newChat?: string; view?: string; file?: string }>;
}
...
export default async function ProjectPage({ searchParams }: ProjectPageProps) {
  const params = await searchParams;
  const { cwd, sessionId, newChat, view, file } = params;
  const initialView = view === 'agent' || view === 'explorer' || view === 'console' ? view : undefined;

  return <TabManager initialCwd={cwd} initialSessionId={sessionId} initialBlank={newChat === '1'} initialView={initialView} initialFile={file} />;
}
