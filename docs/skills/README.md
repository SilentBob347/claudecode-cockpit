...
> Not to be confused with the repo's top-level `skills/`: that one holds the
> **builtin** slash commands (`/qa`, `/cr`, …) that ship inside the npm
> package and are loaded by the server at runtime. Everything here is
...
3. Paste the absolute path to the SKILL.md, e.g. (replace `/path/to/cockpit`
   ...
   ```
   /path/to/cockpit/docs/skills/cockpit-release/SKILL.md
   /path/to/cockpit/docs/skills/cockpit-changelog/SKILL.md
   ```
...
excluded from `package.json#files`). They're project documentation, not
runtime code — unlike the top-level `skills/`, which is runtime code and
*is* published.

## Adding a new skill

1. Create a new folder `docs/skills/cockpit-<name>/` (keep the `cockpit-`
   prefix to avoid colliding with skills from other projects). Do **not**
   put it in the top-level `skills/` — that directory ships to npm.
