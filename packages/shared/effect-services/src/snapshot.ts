...
export interface SnapshotService {
  ...
  /**
   * Commits whose Cockpit-Tool-Id is in `toolIds`, oldest first, narrowed by
   * `sessionKey`.
   *
   * A tool id is unique within a session for every engine — Anthropic's
   * `toolu_…`, codex's app-server item ids, the AI SDK's `call_…` — so the pair
   * identifies exactly one tool call. This used to take a `runId` as well,
   * because codex's old `exec` transport restarted an `item_N` counter each
   * turn; that transport is gone and the extra scope excluded nothing.
   */
