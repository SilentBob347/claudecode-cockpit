/**
 * System prompt for the Built-in Agent loop (engines/builtinAgent).
 *
 * Shared by every provider that runs through our own loop (ollama, deepseek in
 * `mode: 'builtin'`) — it describes OUR tool set, so it must stay
