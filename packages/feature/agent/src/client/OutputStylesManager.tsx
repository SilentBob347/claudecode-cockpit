...
/**
 * Edit the whole output-styles list as one text document (see outputStylesMarkdown.ts
 * for the format). Same shell as QuickInstructionsManager, on purpose: one editing
 * model for both "global" lists.
 */
export function OutputStylesManager({
  styles,
  onSave,
  onClose,
}: {
  styles: OutputStyle[];
  onSave: (styles: OutputStyle[]) => Promise<boolean>;
  onClose: () => void;
}) {
  const { t } = useTranslation();
  const [text, setText] = useState(() => formatOutputStylesMarkdown(styles));
  const [saving, setSaving] = useState(false);
  const [saveFailed, setSaveFailed] = useState(false);
  const parsed = parseOutputStylesMarkdown(text, styles);

  useEscToClose(onClose);

  const save = useCallback(async () => {
    const result = parseOutputStylesMarkdown(text, styles);
    if (!result.ok || saving) return;
    setSaving(true);
    setSaveFailed(false);
    if (await onSave(result.styles)) {
      onClose();
      return;
    }
    setSaving(false);
    setSaveFailed(true);
  }, [onClose, onSave, saving, styles, text]);

  const title = t('chat.editOutputStyles');
        ...
        >
          <header className="flex items-center justify-between px-4 py-3 border-b border-border">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">{t('chat.outputStylesMarkdownHint')}</p>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="min-h-0 flex-1 px-4 py-3">
            <textarea
              ...
              placeholder={t('chat.outputStylesMarkdownPlaceholder')}
              rows={24}
              spellCheck={false}
              data-testid="output-styles-editor"
          ...
          <footer className="flex items-center justify-between gap-3 px-4 py-3 border-t border-border">
            <span className="text-xs text-destructive">
              {!parsed.ok
                ? t(`chat.outputStylesError.${parsed.reason}`, { line: parsed.line })
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onClick={save}
                data-testid="output-styles-save"
