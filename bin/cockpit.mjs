#!/usr/bin/env node
/**
 * `cockpit` — the CLI entry point.
 *
 * Holds the whole command surface: flag parsing, the `--help` text, and the
 * dynamic imports that lazy-load each sub-command module (browser / terminal /
 * codegraph / connection / service). `cockpit-dev` re-imports this file with
 * the dev environment pre-set.
 *
 * `__dirname` must stay `bin/` — PROJECT_ROOT and every `./cockpit-*` import
 * resolve from here.
 */
