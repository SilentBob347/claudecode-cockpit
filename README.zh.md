...
| 图片附件麻烦 | 拖拽 / 粘贴图片直接进对话 |
| "我昨天调的那个 bug 在哪？" | Cmd+K 跨项目会话浏览，会话固定 / 分叉；`/ss` 一句话找回任意历史会话 |
| 每个子代理都从空白开始 | **Bots**：用 `@name` 叫进任务的持久化、文件原生子代理；每次都在可检查的独立会话中运行 |
...
| 工具调用级变更快照（每次 Edit / Write / **Bash** / MCP 调用的真实磁盘 diff，本地保留 7 天） | ✅ | ❌ | ❌ 仅按轮次检查点 |
...
- `/ss` —— 一句话描述，**找回任意历史会话**（跨项目、跨引擎、不限日期），回复里直接给可打开的链接
- `/dl` —— 把子任务**委派**给另一个目录 / 另一个引擎的新会话，不用等；之后随时问进展
- **Bots** —— 用 `@name` 叫进一个持久化、文件原生子代理；它在独立会话中带着自己的身份、上下文、承诺和 Skills 工作
- **工具调用级快照** —— 每次碰文件的工具调用都会打快照（影子 git，全本地）；按 git 历史式时间线逐调用审阅每条回复的变更，`Bash` 改了什么也看得到
...
- 用 `/bot` 创建 Bot，再从 Bots 面板或 `BOT.md` 注册这个普通本地目录
- 用 `@name` 叫它进任务；每轮都在独立、可检查的会话中运行，并返回链接
- 记忆是有意的：普通任务默认只读，只有明确要求记住、更新、纠正或删除时才改文件
- 用 `@name 装上 /path/to/SKILL.md` 关联现有 Skill；用 `@name 复盘一下` 检查过期记忆
- Bot 目录一直是本地、可迁移的 Markdown，没有隐藏数据库、上传或厂商锁定

[查看 Bots 文档](https://opencockpit.dev/zh/docs/agent/bots/)。

## 使用场景

- **独立开发者多仓并行：** "API 在重构、Web 在写测试、Pipeline 在排 bug —— 同时跑、同时可见。"
- **一个真正记得住的专家：** 今天让 `@product` 记下路线图决策，下个月再叫它时读的还是同一份显式、可审阅上下文，不污染主会话。
...
[![在线体验](https://img.shields.io/badge/%E5%9C%A8%E7%BA%BF%E4%BD%93%E9%AA%8C-opencockpit.dev%2Ftry-12a594?style=for-the-badge)](https://opencockpit.dev/try)
