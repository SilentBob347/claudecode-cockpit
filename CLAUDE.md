...
- `/public/html-lib/` - Same-origin hosted frontend libs for HTML apps
  ...
  markdown / json / csv / pdf widget bundles). Regenerated by
  `scripts/build-html-lib.mjs` on every `predev` / `prebuild`
- `/skills/` - Builtin slash-command prompts, one `SKILL.md` per command
  (`/qa`, `/cr`, …). English only, **no TypeScript involved**: the directory
  set IS the registry — both dispatch (`slashCommands.ts`) and the
  autocomplete route read it, so adding a command is `mkdir skills/<cmd>` +
  one file. Shipped in `package.json#files`, so anything added here lands in
